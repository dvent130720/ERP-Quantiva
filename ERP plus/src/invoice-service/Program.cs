...
builder.Services.AddCommonPlatform(builder.Configuration, "invoice-service");
...
app.MapGet("/api/invoice", async (AppDbContext db, ITenantProvider tenant) =>
{
...
app.MapPost("/api/invoice", async (CreateRecordCommand cmd, ISender sender) =>
{
    var result = await sender.Send(cmd);
    return Results.Created($"/api/invoice/{result.Id}", result);
...
public sealed class CreateRecordHandler(AppDbContext db, ITenantProvider tenant, IValidator<CreateRecordCommand> validator, IPublishEndpoint publish) : IRequestHandler<CreateRecordCommand, RecordEntity>
{
    public async Task<RecordEntity> Handle(CreateRecordCommand request, CancellationToken cancellationToken)
    {
        ...
        await publish.Publish(new DomainEvent($"invoice-serviceCreated", entity.Id, entity.TenantId), cancellationToken);
        return entity;
    }
}
