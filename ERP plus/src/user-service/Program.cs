...
builder.Services.AddCommonPlatform(builder.Configuration, "user-service");
...
app.MapGet("/api/user", async (AppDbContext db, ITenantProvider tenant) =>
{
...
app.MapPost("/api/user", async (CreateRecordCommand cmd, ISender sender) =>
{
...
public sealed class CreateRecordHandler(AppDbContext db, ITenantProvider tenant, IValidator<CreateRecordCommand> validator, IPublishEndpoint publish) : IRequestHandler<CreateRecordCommand, RecordEntity>
{
    public async Task<RecordEntity> Handle(CreateRecordCommand request, CancellationToken cancellationToken)
    {
        ...
        await publish.Publish(new DomainEvent($"user-serviceCreated", entity.Id, entity.TenantId), cancellationToken);
        return entity;
    }
}
