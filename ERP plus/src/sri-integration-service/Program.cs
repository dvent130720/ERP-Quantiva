...
builder.Services.AddCommonPlatform(builder.Configuration, "sri-integration-service");
...
app.MapGet("/api/sri", async (AppDbContext db, ITenantProvider tenant) =>
{
...
app.MapPost("/api/sri", async (CreateRecordCommand cmd, ISender sender) =>
{
    var result = await sender.Send(cmd);
    return Results.Created($"/api/sri/{result.Id}", result);
...
public sealed class CreateRecordHandler(AppDbContext db, ITenantProvider tenant, IValidator<CreateRecordCommand> validator, IPublishEndpoint publish) : IRequestHandler<CreateRecordCommand, RecordEntity>
{
    public async Task<RecordEntity> Handle(CreateRecordCommand request, CancellationToken cancellationToken)
    {
        ...
        await publish.Publish(new DomainEvent($"sri-integration-serviceCreated", entity.Id, entity.TenantId), cancellationToken);
        return entity;
    }
}
