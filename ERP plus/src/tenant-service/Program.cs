...
builder.Services.AddCommonPlatform(builder.Configuration, "tenant-service");
...
app.MapGet("/api/tenant", async (AppDbContext db, ITenantProvider tenant) =>
{
...
app.MapPost("/api/tenant", async (CreateRecordCommand cmd, ISender sender) =>
{
    var result = await sender.Send(cmd);
    return Results.Created($"/api/tenant/{result.Id}", result);
...
public sealed class CreateRecordHandler(AppDbContext db, ITenantProvider tenant, IValidator<CreateRecordCommand> validator, IPublishEndpoint publish) : IRequestHandler<CreateRecordCommand, RecordEntity>
{
    public async Task<RecordEntity> Handle(CreateRecordCommand request, CancellationToken cancellationToken)
    {
        ...
        await publish.Publish(new DomainEvent($"tenant-serviceCreated", entity.Id, entity.TenantId), cancellationToken);
        return entity;
    }
}
