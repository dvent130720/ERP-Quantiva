...
builder.Services.AddCommonPlatform(builder.Configuration, "notification-service");
...
app.MapGet("/api/notification", async (AppDbContext db, ITenantProvider tenant) =>
{
...
app.MapPost("/api/notification", async (CreateRecordCommand cmd, ISender sender) =>
{
    var result = await sender.Send(cmd);
    return Results.Created($"/api/notification/{result.Id}", result);
...
public sealed class CreateRecordHandler(AppDbContext db, ITenantProvider tenant, IValidator<CreateRecordCommand> validator, IPublishEndpoint publish) : IRequestHandler<CreateRecordCommand, RecordEntity>
{
    public async Task<RecordEntity> Handle(CreateRecordCommand request, CancellationToken cancellationToken)
    {
        ...
        await publish.Publish(new DomainEvent($"notification-serviceCreated", entity.Id, entity.TenantId), cancellationToken);
        return entity;
    }
}
