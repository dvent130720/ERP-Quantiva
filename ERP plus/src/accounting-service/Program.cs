...
builder.Services.AddCommonPlatform(builder.Configuration, "accounting-service");
...
app.MapGet("/api/accounting", async (AppDbContext db, ITenantProvider tenant) =>
{
...
app.MapPost("/api/accounting", async (CreateRecordCommand cmd, ISender sender) =>
{
    var result = await sender.Send(cmd);
    return Results.Created($"/api/accounting/{result.Id}", result);
...
public sealed class CreateRecordHandler(AppDbContext db, ITenantProvider tenant, IValidator<CreateRecordCommand> validator, IPublishEndpoint publish) : IRequestHandler<CreateRecordCommand, RecordEntity>
{
    public async Task<RecordEntity> Handle(CreateRecordCommand request, CancellationToken cancellationToken)
    {
        ...
        await publish.Publish(new DomainEvent($"accounting-serviceCreated", entity.Id, entity.TenantId), cancellationToken);
        return entity;
    }
}
