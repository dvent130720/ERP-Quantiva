...
builder.Services.AddCommonPlatform(builder.Configuration, "ai-service");
...
app.MapGet("/api/ai", async (AppDbContext db, ITenantProvider tenant) =>
{
...
app.MapPost("/api/ai", async (CreateRecordCommand cmd, ISender sender) =>
{
    var result = await sender.Send(cmd);
    return Results.Created($"/api/ai/{result.Id}", result);
...
public sealed class CreateRecordHandler(AppDbContext db, ITenantProvider tenant, IValidator<CreateRecordCommand> validator, IPublishEndpoint publish) : IRequestHandler<CreateRecordCommand, RecordEntity>
{
    public async Task<RecordEntity> Handle(CreateRecordCommand request, CancellationToken cancellationToken)
    {
        ...
        await publish.Publish(new DomainEvent($"ai-serviceCreated", entity.Id, entity.TenantId), cancellationToken);
        return entity;
    }
}
