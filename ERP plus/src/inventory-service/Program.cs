...
builder.Services.AddCommonPlatform(builder.Configuration, "inventory-service");
...
app.MapGet("/api/inventory", async (AppDbContext db, ITenantProvider tenant) =>
{
...
app.MapPost("/api/inventory", async (CreateRecordCommand cmd, ISender sender) =>
{
...
public sealed class CreateRecordHandler(AppDbContext db, ITenantProvider tenant, IValidator<CreateRecordCommand> validator, IPublishEndpoint publish) : IRequestHandler<CreateRecordCommand, RecordEntity>
{
    public async Task<RecordEntity> Handle(CreateRecordCommand request, CancellationToken cancellationToken)
    {
        ...
        await publish.Publish(new DomainEvent($"inventory-serviceCreated", entity.Id, entity.TenantId), cancellationToken);
        return entity;
    }
}
